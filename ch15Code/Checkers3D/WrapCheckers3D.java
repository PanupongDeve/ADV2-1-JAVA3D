...
// Holds the 3D canvas where the loaded image is displayed
{
  ...
  // A shiny blue sphere located at (0,4,0)
  {
    ...
    Material tailPencil = new Material(red, red, red, specular, 25.0f);
       // sets ambient, emissive, diffuse, specular, shininess
    tailPencil.setLightingEnable(true);

    Material headPencil = new Material(gray, gray, gray, specular, 25.0f);
    headPencil.setLightingEnable(true);

    Appearance tailPancilApp = new Appearance();
    tailPancilApp.setMaterial(tailPencil);

    Appearance headPencilApp = new Appearance();
    headPencilApp.setMaterial(headPencil);

    // position the sphere
    Transform3D t3d_head = new Transform3D();
    t3d_head.set( new Vector3f(0,2f,0));
    t3d_head.rotZ(3.14); 
    TransformGroup tg_head = new TransformGroup(t3d_head);
    tg_head.addChild(new Sphere(2.0f, headPencilApp)); 

    Transform3D t3d_tail = new Transform3D();
    t3d_tail.set( new Vector3f(0,4,0)); 
    TransformGroup tg_tail = new TransformGroup(t3d_tail);
    tg_tail.addChild(new Cylinder(2.0f, tailPancilApp));   // set its radius and appearance
 
    sceneBG.addChild(t3d_head);
    sceneBG.addChild(t3d_tail);
